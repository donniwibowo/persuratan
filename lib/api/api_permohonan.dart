...
class ApiPermohonan extends ChangeNotifier {
  ...
  Future<List<PermohonanModel>> getAllPermohonan(String keyword) async {
    ...
    final api_url =
        'https://192.168.1.28/leap_integra/master/dms/api/form/getpermohonan?user_token=' +
            user_token +
            '&search_keyword=' +
            keyword;
  ...
  Future<List<PermohonanModel>> getDetailPermohonan(
    ...
    final api_url =
        'https://192.168.1.28/leap_integra/master/dms/api/form/getdetailpermohonan?user_token=' +
            user_token +
            '&permohonan_id=' +
            permohonan_id;
    final response = await http.get(Uri.parse(api_url));
